...
export function applyHookContrastRevision(input){
  if(input?.episodeId!==EPISODE_ID) throw new Error(`episode mismatch: expected ${EPISODE_ID}, found ${input?.episodeId||'null'}`);
  return {...input,hookContrastRevision:HOOK_CONTRAST_REVISION};
}

export function planEpisode1HookContrastRepair({state,input}){
  if(input?.episodeId!==EPISODE_ID||state?.episode_id!==EPISODE_ID) return {kind:'NOOP',reason:'another_episode'};
  if(input?.hookContrastRevision===HOOK_CONTRAST_REVISION) return {kind:'NOOP',reason:'hook_contrast_revision_already_applied'};
  ...
  return {
    ...
    reason:'rerender after QA inspection found unreadable recovered-hours hook label; preserve bespoke visuals and corrected thumbnail framing',
  };
}
...
function main(){
  if(!fs.existsSync(STATE_PATH)||!fs.existsSync(INPUT_PATH)){
    console.log('EPISODE1_HOOK_CONTRAST_REPAIR_NOOP missing_state_or_input');
    return;
  }
  ...
  if(plan.kind!=='REBUILD'){
    console.log(`EPISODE1_HOOK_CONTRAST_REPAIR_${plan.kind} ${plan.reason}`);
    return;
  }
  writeJson(INPUT_PATH,applyHookContrastRevision(input));
  const patched=runState(['patch',String(plan.expectedRevision),plan.episodeId,'episode1-hook-contrast-repair',JSON.stringify({production:plan.productionPatch})]);
  const transitioned=runState(['transition',String(patched.state_revision),plan.episodeId,plan.transitionTo,'episode1-hook-contrast-repair',plan.reason]);
  console.log(`EPISODE1_HOOK_CONTRAST_REPAIR_READY revision=${transitioned.state_revision}`);
}

if(import.meta.url===`file://${process.argv[1]}`){
  try{main();}catch(error){console.error(`EPISODE1_HOOK_CONTRAST_REPAIR_ERROR ${error?.stack||String(error)}`);process.exitCode=1;}
}
